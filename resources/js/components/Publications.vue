          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Publication Table</h3>
              ...
              <div class="card-body table-responsive p-0">
                <table class="table table-hover text-nowrap">
                  ...
                  <tbody>
                    <tr v-for="(publication, index) in publications.data" :key="publication.id">
                      <td :title="publication.id">
                        {{ page + index + 1 }}
                      </td>
                      <td>{{ publication.filename }}</td>
                      <td>{{ publication.desc }}</td>
                      <td>{{ publication.unit }}</td>
                      <td>
                        {{ publication.type }}
                      </td>
                      <td>
                      ...
                      <td>
                        <a href="#" title="Edit" @click="openUserModal(publication)">
                          <i class="fa fa-edit indigo"></i>
                        </a>
                        <span class="yellow">/</span>
                        <a href="#" @click.prevent="deleteUser(publication)" title="Remove">
  ...
  export default {
    ...
    methods: {
      ...
      async getUsers(page = 1) {
        ...
        try {
          const publications = await axios.get(`/api/publications/?page=${page}`);
      ...
      async createUser() {
        ...
        try {
          ...
          axios.post('api/publications',fd,config ).then(res=>{
        ...
        try {
          await this.form.put(`/api/publications/${this.form.id}`);
          $("#userModal").modal("hide");
      ...
      async deleteUser(user) {
        ...
        try {
          ...
          if (result.isConfirmed) {
            await this.form.delete(`/api/publications/${user.id}`);
