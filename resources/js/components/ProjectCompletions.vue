          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Project Completion Table</h3>
              ...
              <div class="card-body table-responsive p-0">
                <table class="table table-hover text-nowrap">
                  ...
                  <tbody>
                    <tr v-for="(projectcompletion, index) in projectcompletions.data" :key="projectcompletion.id">
                      <td :title="projectcompletion.id">
                        {{ page + index + 1 }}
                      </td>
                      <td>{{ projectcompletion.filename }}</td>
                      <td>{{ projectcompletion.desc }}</td>
                      <td>{{ projectcompletion.unit }}</td>
                      <td>
                        {{ projectcompletion.type }}
                      </td>
                      <td>
                       {{ projectcompletion.uploader }}
                      </td>
                      <td>
                        <a href="#" title="Edit" @click="openUserModal(projectcompletion)">
                          <i class="fa fa-edit indigo"></i>
                        </a>
                        <span class="yellow">/</span>
                        <a href="#" @click.prevent="deleteUser(projectcompletion)" title="Remove">
  ...
  export default {
    ...
    methods: {
      ...
      async getUsers(page = 1) {
        ...
        try {
          const projectcompletions = await axios.get(`/api/projectcompletions/?page=${page}`);
          this.projectcompletions = projectcompletions.data;
        } catch (error) {
      ...
      async createUser() {
        ...
        try {
          ...
          axios.post('api/projectcompletions',fd,config ).then(res=>{
        ...
        try {
          await this.form.put(`/api/projectcompletions/${this.form.id}`);
          $("#userModal").modal("hide");
      ...
      async deleteUser(user) {
        ...
        try {
          ...
          if (result.isConfirmed) {
            await this.form.delete(`/api/projectcompletions/${user.id}`);
      ...
      window.Fire.$on("search", (search) => {
        ...
        axios
          ...
          .then((data) => {
            ...
            this.projectcompletions = data.data;
          })
          .catch((e) => {
            console.log(e);
          });
