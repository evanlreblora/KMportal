              ...
              <table class="table table-hover text-nowrap">
                ...
                <tbody>
                  <tr v-for="(video, index) in videos.data" :key="video.id">
                    ...
                    <td>
                      <a href="#" title="Edit" @click="openUserModal(video)">
                        <i class="fa fa-edit indigo"></i>
                      </a>
                      <span class="yellow">/</span>
                      <a href="#" @click.prevent="deleteUser(video)" title="Remove">
...
export default {
  ...
  methods: {
    ...
    async getUsers(page = 1) {
      ...
      try {
        const videos = await axios.get(`/api/videos/?page=${page}`);
    ...
    async createUser() {
      ...
      try {
        ...
        axios.post('api/videos',fd,config ).then(res=>{
      ...
      try {
        await this.form.put(`/api/videos/${this.form.id}`);
        $("#userModal").modal("hide");
    ...
    async deleteUser(user) {
      ...
      try {
        ...
        if (result.isConfirmed) {
          await this.form.delete(`/api/videos/${user.id}`);
