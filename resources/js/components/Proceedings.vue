        ...
        <div class="col-md-12">
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">Proceeding Table</h3>
            ...
            <div class="card-body table-responsive p-0">
              <table class="table table-hover text-nowrap">
                ...
                <tbody>
                  <tr v-for="(proceeding, index) in proceedings.data" :key="proceeding.id">
                    <td :title="proceeding.id">
                      {{ page + index + 1 }}
                    </td>
                    <td>{{ proceeding.filename }}</td>
                    <td>{{ proceeding.desc }}</td>
                    <td>{{ proceeding.unit }}</td>
                    <td>
                      {{ proceeding.type }}
                    </td>
                    <td>
                    ...
                    <td>
                      <a href="#" title="Edit" @click="openUserModal(proceeding)">
                        <i class="fa fa-edit indigo"></i>
                      </a>
                      <span class="yellow">/</span>
                      <a href="#" @click.prevent="deleteUser(proceeding)" title="Remove">
...
export default {
  ...
  methods: {
    ...
    async getUsers(page = 1) {
      ...
      try {
        const proceedings = await axios.get(`/api/proceedings/?page=${page}`);
        this.proceedings = proceedings.data;
      } catch (error) {
    ...
    async createUser() {
      ...
      try {
        ...
        axios.post('api/proceedings',fd,config ).then(res=>{
      ...
      try {
        await this.form.put(`/api/proceedings/${this.form.id}`);
        $("#userModal").modal("hide");
    ...
    async deleteUser(user) {
      ...
      try {
        ...
        if (result.isConfirmed) {
          await this.form.delete(`/api/proceedings/${user.id}`);
